...
The repo had a generic VPS deployment checklist, a tracked `.env.prod` file with production-style values, and a GitHub Actions workflow that deployed with `.env` even though the production Compose stack expects `.env.prod`.
...
- Make the VPS deploy workflow use `/opt/ds-general/.env.prod`.
...
- Remove the tracked `.env.prod` file.
- Keep `.env.prod.example` as the only tracked production template.
- Update the production template to the single-domain DS General deployment shape.

Tests for this phase:

- Confirm `.env.prod` is no longer tracked.
...
- Update the GitHub Actions deploy workflow to:
  ...
  - build and start with `docker compose --env-file .env.prod -f docker-compose.prod.yml`
...
- Confirm the shell script references `.env.prod` consistently.
...
- [x] Remove the repo-root `.env.prod` file and keep `.env.prod.example` as the production template.
- [x] Update `.env.prod.example` for the DS General single-domain production contract.
- [x] Update the deploy workflow to use `/opt/ds-general/.env.prod`.
...
- [ ] User creates `/opt/ds-general/.env.prod` on the VPS and completes the external rollout steps.
