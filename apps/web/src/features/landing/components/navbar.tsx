...
export function Navbar() {
  ...
  const desktopActivePathItemLabel = navItems.find(
    (item) => item.path && isActive(item.path),
  )?.label;
  const primaryDesktopIndicatorLabel =
        ...
        <nav className="hidden items-center lg:flex">
          {navItems.map((item) => {
            if (item.hasDropdown) {
              ...
              const showSharedIndicator =
                primaryDesktopIndicatorLabel === item.label;
                ...
                <div key={item.label} ref={dropdownRef} className="relative">
                  ...
                  >
                    {isDropdownActive ? (
                      showSharedIndicator ? (
                        <motion.span
                          layoutId="landing-desktop-nav-indicator"
                    ...
                    <span className="relative z-10 flex items-center gap-2.5">
                      ...
                      >
                        <path
                          ...
                          fill={
                            isDropdownActive ? "white" : "var(--foreground)"
                          }
                        />
                      </svg>
                    </span>
            ...
            return (
              <Link
                key={item.label}
                to={item.path}
                className={`relative isolate flex items-center gap-2.5 overflow-hidden px-4 py-2 font-sans text-[14px] font-medium no-underline transition-colors ${
                  linkActive
                    ? "text-primary-foreground"
                    : "text-foreground hover:text-primary"
                }`}
      ...
      <AnimatePresence initial={false}>
        {mobileOpen ? (
          <motion.nav
            ...
            className="flex flex-col gap-2 overflow-hidden border-t border-border/60 bg-background px-6 py-4 lg:hidden"
          >
            {navItems.map((item) => {
              if (item.hasDropdown) {
                return (
